...
class EmulatorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("RPCS5 Emulator PoC")
    ...
    def setup_ui(self):
        ...
        self.cpu_label = ttk.Label(perf_frame, text="CPU Usage: 0%")
        self.cpu_label.pack(side=tk.LEFT, padx=10)
        self.ram_label = ttk.Label(perf_frame, text="RAM Usage: 0 MB")
    ...
    def update_canvas(self):
        ...
        self.canvas.create_text(280, 90, text="RPCS5 Emulator PoC", fill="white", font=('Segoe UI', 16))
        self.canvas.create_text(10, 10, text=f"PC: 0x{self.emulator.cpu.pc:016x}", fill="white", font=('Segoe UI', 10), anchor='nw')
        regs = ', '.join(map(str, self.emulator.cpu.registers[:8]))
        self.canvas.create_text(10, 30, text=f"Registers: {regs}...", fill="white", font=('Segoe UI', 10), anchor='nw')
